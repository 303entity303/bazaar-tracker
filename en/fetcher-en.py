...
print("📈 To get accurate predictions later, it is strongly recommended to keep this script running continuously.")
...
interval_seconds = 20  # Changed variable name for clarity
output_dir = "prices"
...
try:
    ...
    # === MAIN DATA COLLECTION LOOP FOR ALL ITEMS ===
    while True:
        try:
            now_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
            url = "https://api.hypixel.net/v2/skyblock/bazaar"
            # response = requests.get(url, headers={"API-Key": api_key}) # API Key commented out
            response = requests.get(url) # Removed API Key header
            ...
            if response.status_code == 200:
                ...
                for product_id, product_data in all_products.items():
                    ...
                    # print(f"[{record['timestamp_utc']}] Saved: {product_id} → buy {record['buyPrice']} / sell {record['sellPrice']}")

            else:
                print(f"[{datetime.utcnow().isoformat()}] ❌ HTTP Error: {response.status_code}")

        except Exception as e:
            print(f"[{datetime.utcnow().isoformat()}] ❗ Exception: {e}")

        time.sleep(interval_seconds) # Use translated variable name
